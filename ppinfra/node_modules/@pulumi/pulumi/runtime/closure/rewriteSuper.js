...
function getFactory(transformationContext) {
    ...
    const transformationContextFactory = transformationContext.factory;
    ...
    function updateFunctionDeclaration(node, decorators, modifiers, asteriskToken, name, typeParameters, parameters, type, body) {
        if (tsLessThan4) {
            return ts.updateFunctionDeclaration(node, decorators, modifiers, asteriskToken, name, typeParameters, parameters, type, body);
        }
        else if (tsLessThan48) {
            return transformationContextFactory.updateFunctionDeclaration(node, decorators, modifiers, asteriskToken, name, typeParameters, parameters, type, body);
        }
        else {
            return transformationContextFactory.updateFunctionDeclaration(node, modifiers, asteriskToken, name, typeParameters, parameters, type, body);
        }
    }
    return {
    ...
    function rewriteSuperCallsWorker(transformationContext) {
        ...
        function visitor(node) {
            ...
            if (firstFunctionDeclaration && ts.isFunctionDeclaration(node)) {
                ...
                return factory.updateFunctionDeclaration(funcDecl, funcDecl.decorators, funcDecl.modifiers, funcDecl.asteriskToken, factory.createIdentifier(text), funcDecl.typeParameters, funcDecl.parameters, funcDecl.type, funcDecl.body);
            }
            ...
            if (ts.isCallExpression(rewritten) && newNodes.has(rewritten.expression)) {
                ...
                return factory.updateCallExpression(rewritten, factory.createPropertyAccessExpression(rewritten.expression, "call"), rewritten.typeArguments, argumentsCopy);
            }
            return rewritten;
        }
