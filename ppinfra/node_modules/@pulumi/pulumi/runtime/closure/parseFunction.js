...
function parseFunctionCode(funcString) {
    ...
    if (funcString.startsWith("class ")) {
        ...
        if (!constructor) {
            ...
            const isSubClass = (_a = classDecl.heritageClauses) === null || _a === void 0 ? void 0 : _a.some((c) => c.token === ts.SyntaxKind.ExtendsKeyword);
            return isSubClass
                ? makeFunctionDeclaration("constructor() { super(); }", 
                /*isAsync:*/ false, 
...
function computeCapturedVariableNames(file) {
    ...
    function determineCapturedPropertyChain(node) {
        ...
        while ((node === null || node === void 0 ? void 0 : node.parent) && isPropertyOrElementAccessExpression(node.parent) && node.parent.expression === node) {
            if (!infos) {
                infos = [];
            }
    ...
    function visitVariableDeclaration(node) {
        ...
        const isLet = node.parent !== undefined &&
            ts.isVariableDeclarationList(node.parent) &&
            (node.parent.flags & ts.NodeFlags.Let) !== 0;
        // eslint-disable-next-line max-len
        const isConst = node.parent !== undefined &&
            ts.isVariableDeclarationList(node.parent) &&
