"use strict";
// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __exportStar = (this && this.__exportStar) || function(m, exports) {
    for (var p in m) if (p !== "default" && !Object.prototype.hasOwnProperty.call(exports, p)) __createBinding(exports, m, p);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ServiceSetting = exports.ResourceDataSync = exports.QuicksetupConfigurationManager = exports.PatchGroup = exports.PatchBaseline = exports.Parameter = exports.MaintenanceWindowTask = exports.MaintenanceWindowTarget = exports.MaintenanceWindow = exports.getPatchBaselinesOutput = exports.getPatchBaselines = exports.getPatchBaselineOutput = exports.getPatchBaseline = exports.getParametersByPathOutput = exports.getParametersByPath = exports.getParameterOutput = exports.getParameter = exports.getMaintenanceWindowsOutput = exports.getMaintenanceWindows = exports.getInstancesOutput = exports.getInstances = exports.getDocumentOutput = exports.getDocument = exports.getContactsRotationOutput = exports.getContactsRotation = exports.Document = exports.DefaultPatchBaseline = exports.ContactsRotation = exports.Association = exports.Activation = void 0;
const pulumi = require("@pulumi/pulumi");
const utilities = require("../utilities");
exports.Activation = null;
utilities.lazyLoad(exports, ["Activation"], () => require("./activation"));
exports.Association = null;
utilities.lazyLoad(exports, ["Association"], () => require("./association"));
exports.ContactsRotation = null;
utilities.lazyLoad(exports, ["ContactsRotation"], () => require("./contactsRotation"));
exports.DefaultPatchBaseline = null;
utilities.lazyLoad(exports, ["DefaultPatchBaseline"], () => require("./defaultPatchBaseline"));
exports.Document = null;
utilities.lazyLoad(exports, ["Document"], () => require("./document"));
exports.getContactsRotation = null;
exports.getContactsRotationOutput = null;
utilities.lazyLoad(exports, ["getContactsRotation", "getContactsRotationOutput"], () => require("./getContactsRotation"));
exports.getDocument = null;
exports.getDocumentOutput = null;
utilities.lazyLoad(exports, ["getDocument", "getDocumentOutput"], () => require("./getDocument"));
exports.getInstances = null;
exports.getInstancesOutput = null;
utilities.lazyLoad(exports, ["getInstances", "getInstancesOutput"], () => require("./getInstances"));
exports.getMaintenanceWindows = null;
exports.getMaintenanceWindowsOutput = null;
utilities.lazyLoad(exports, ["getMaintenanceWindows", "getMaintenanceWindowsOutput"], () => require("./getMaintenanceWindows"));
exports.getParameter = null;
exports.getParameterOutput = null;
utilities.lazyLoad(exports, ["getParameter", "getParameterOutput"], () => require("./getParameter"));
exports.getParametersByPath = null;
exports.getParametersByPathOutput = null;
utilities.lazyLoad(exports, ["getParametersByPath", "getParametersByPathOutput"], () => require("./getParametersByPath"));
exports.getPatchBaseline = null;
exports.getPatchBaselineOutput = null;
utilities.lazyLoad(exports, ["getPatchBaseline", "getPatchBaselineOutput"], () => require("./getPatchBaseline"));
exports.getPatchBaselines = null;
exports.getPatchBaselinesOutput = null;
utilities.lazyLoad(exports, ["getPatchBaselines", "getPatchBaselinesOutput"], () => require("./getPatchBaselines"));
exports.MaintenanceWindow = null;
utilities.lazyLoad(exports, ["MaintenanceWindow"], () => require("./maintenanceWindow"));
exports.MaintenanceWindowTarget = null;
utilities.lazyLoad(exports, ["MaintenanceWindowTarget"], () => require("./maintenanceWindowTarget"));
exports.MaintenanceWindowTask = null;
utilities.lazyLoad(exports, ["MaintenanceWindowTask"], () => require("./maintenanceWindowTask"));
exports.Parameter = null;
utilities.lazyLoad(exports, ["Parameter"], () => require("./parameter"));
__exportStar(require("./parameterType"), exports);
exports.PatchBaseline = null;
utilities.lazyLoad(exports, ["PatchBaseline"], () => require("./patchBaseline"));
exports.PatchGroup = null;
utilities.lazyLoad(exports, ["PatchGroup"], () => require("./patchGroup"));
exports.QuicksetupConfigurationManager = null;
utilities.lazyLoad(exports, ["QuicksetupConfigurationManager"], () => require("./quicksetupConfigurationManager"));
exports.ResourceDataSync = null;
utilities.lazyLoad(exports, ["ResourceDataSync"], () => require("./resourceDataSync"));
exports.ServiceSetting = null;
utilities.lazyLoad(exports, ["ServiceSetting"], () => require("./serviceSetting"));
// Export enums:
__exportStar(require("../types/enums/ssm"), exports);
const _module = {
    version: utilities.getVersion(),
    construct: (name, type, urn) => {
        switch (type) {
            case "aws:ssm/activation:Activation":
                return new exports.Activation(name, undefined, { urn });
            case "aws:ssm/association:Association":
                return new exports.Association(name, undefined, { urn });
            case "aws:ssm/contactsRotation:ContactsRotation":
                return new exports.ContactsRotation(name, undefined, { urn });
            case "aws:ssm/defaultPatchBaseline:DefaultPatchBaseline":
                return new exports.DefaultPatchBaseline(name, undefined, { urn });
            case "aws:ssm/document:Document":
                return new exports.Document(name, undefined, { urn });
            case "aws:ssm/maintenanceWindow:MaintenanceWindow":
                return new exports.MaintenanceWindow(name, undefined, { urn });
            case "aws:ssm/maintenanceWindowTarget:MaintenanceWindowTarget":
                return new exports.MaintenanceWindowTarget(name, undefined, { urn });
            case "aws:ssm/maintenanceWindowTask:MaintenanceWindowTask":
                return new exports.MaintenanceWindowTask(name, undefined, { urn });
            case "aws:ssm/parameter:Parameter":
                return new exports.Parameter(name, undefined, { urn });
            case "aws:ssm/patchBaseline:PatchBaseline":
                return new exports.PatchBaseline(name, undefined, { urn });
            case "aws:ssm/patchGroup:PatchGroup":
                return new exports.PatchGroup(name, undefined, { urn });
            case "aws:ssm/quicksetupConfigurationManager:QuicksetupConfigurationManager":
                return new exports.QuicksetupConfigurationManager(name, undefined, { urn });
            case "aws:ssm/resourceDataSync:ResourceDataSync":
                return new exports.ResourceDataSync(name, undefined, { urn });
            case "aws:ssm/serviceSetting:ServiceSetting":
                return new exports.ServiceSetting(name, undefined, { urn });
            default:
                throw new Error(`unknown resource type ${type}`);
        }
    },
};
pulumi.runtime.registerResourceModule("aws", "ssm/activation", _module);
pulumi.runtime.registerResourceModule("aws", "ssm/association", _module);
pulumi.runtime.registerResourceModule("aws", "ssm/contactsRotation", _module);
pulumi.runtime.registerResourceModule("aws", "ssm/defaultPatchBaseline", _module);
pulumi.runtime.registerResourceModule("aws", "ssm/document", _module);
pulumi.runtime.registerResourceModule("aws", "ssm/maintenanceWindow", _module);
pulumi.runtime.registerResourceModule("aws", "ssm/maintenanceWindowTarget", _module);
pulumi.runtime.registerResourceModule("aws", "ssm/maintenanceWindowTask", _module);
pulumi.runtime.registerResourceModule("aws", "ssm/parameter", _module);
pulumi.runtime.registerResourceModule("aws", "ssm/patchBaseline", _module);
pulumi.runtime.registerResourceModule("aws", "ssm/patchGroup", _module);
pulumi.runtime.registerResourceModule("aws", "ssm/quicksetupConfigurationManager", _module);
pulumi.runtime.registerResourceModule("aws", "ssm/resourceDataSync", _module);
pulumi.runtime.registerResourceModule("aws", "ssm/serviceSetting", _module);
//# sourceMappingURL=index.js.map