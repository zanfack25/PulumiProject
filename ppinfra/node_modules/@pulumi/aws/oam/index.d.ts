export { GetLinkArgs, GetLinkResult, GetLinkOutputArgs } from "./getLink";
export declare const getLink: typeof import("./getLink").getLink;
export declare const getLinkOutput: typeof import("./getLink").getLinkOutput;
export { GetLinksResult } from "./getLinks";
export declare const getLinks: typeof import("./getLinks").getLinks;
export declare const getLinksOutput: typeof import("./getLinks").getLinksOutput;
export { GetSinkArgs, GetSinkResult, GetSinkOutputArgs } from "./getSink";
export declare const getSink: typeof import("./getSink").getSink;
export declare const getSinkOutput: typeof import("./getSink").getSinkOutput;
export { GetSinksResult } from "./getSinks";
export declare const getSinks: typeof import("./getSinks").getSinks;
export declare const getSinksOutput: typeof import("./getSinks").getSinksOutput;
export { LinkArgs, LinkState } from "./link";
export type Link = import("./link").Link;
export declare const Link: typeof import("./link").Link;
export { SinkArgs, SinkState } from "./sink";
export type Sink = import("./sink").Sink;
export declare const Sink: typeof import("./sink").Sink;
export { SinkPolicyArgs, SinkPolicyState } from "./sinkPolicy";
export type SinkPolicy = import("./sinkPolicy").SinkPolicy;
export declare const SinkPolicy: typeof import("./sinkPolicy").SinkPolicy;
