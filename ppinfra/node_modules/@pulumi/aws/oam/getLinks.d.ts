import * as pulumi from "@pulumi/pulumi";
/**
 * Data source for managing an AWS CloudWatch Observability Access Manager Links.
 *
 * ## Example Usage
 *
 * ### Basic Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as aws from "@pulumi/aws";
 *
 * const example = aws.oam.getLinks({});
 * ```
 */
export declare function getLinks(opts?: pulumi.InvokeOptions): Promise<GetLinksResult>;
/**
 * A collection of values returned by getLinks.
 */
export interface GetLinksResult {
    /**
     * Set of ARN of the Links.
     */
    readonly arns: string[];
    /**
     * The provider-assigned unique ID for this managed resource.
     */
    readonly id: string;
}
/**
 * Data source for managing an AWS CloudWatch Observability Access Manager Links.
 *
 * ## Example Usage
 *
 * ### Basic Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as aws from "@pulumi/aws";
 *
 * const example = aws.oam.getLinks({});
 * ```
 */
export declare function getLinksOutput(opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetLinksResult>;
