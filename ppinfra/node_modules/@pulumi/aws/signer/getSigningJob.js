"use strict";
// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***
Object.defineProperty(exports, "__esModule", { value: true });
exports.getSigningJobOutput = exports.getSigningJob = void 0;
const pulumi = require("@pulumi/pulumi");
const utilities = require("../utilities");
/**
 * Provides information about a Signer Signing Job.
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as aws from "@pulumi/aws";
 *
 * const buildSigningJob = aws.signer.getSigningJob({
 *     jobId: "9ed7e5c3-b8d4-4da0-8459-44e0b068f7ee",
 * });
 * ```
 */
function getSigningJob(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("aws:signer/getSigningJob:getSigningJob", {
        "jobId": args.jobId,
    }, opts);
}
exports.getSigningJob = getSigningJob;
/**
 * Provides information about a Signer Signing Job.
 *
 * ## Example Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as aws from "@pulumi/aws";
 *
 * const buildSigningJob = aws.signer.getSigningJob({
 *     jobId: "9ed7e5c3-b8d4-4da0-8459-44e0b068f7ee",
 * });
 * ```
 */
function getSigningJobOutput(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("aws:signer/getSigningJob:getSigningJob", {
        "jobId": args.jobId,
    }, opts);
}
exports.getSigningJobOutput = getSigningJobOutput;
//# sourceMappingURL=getSigningJob.js.map