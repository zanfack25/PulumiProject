export { GetAssetArgs, GetAssetResult, GetAssetOutputArgs } from "./getAsset";
export declare const getAsset: typeof import("./getAsset").getAsset;
export declare const getAssetOutput: typeof import("./getAsset").getAssetOutput;
export { GetAssetsArgs, GetAssetsResult, GetAssetsOutputArgs } from "./getAssets";
export declare const getAssets: typeof import("./getAssets").getAssets;
export declare const getAssetsOutput: typeof import("./getAssets").getAssetsOutput;
export { GetOutpostArgs, GetOutpostResult, GetOutpostOutputArgs } from "./getOutpost";
export declare const getOutpost: typeof import("./getOutpost").getOutpost;
export declare const getOutpostOutput: typeof import("./getOutpost").getOutpostOutput;
export { GetOutpostInstanceTypeArgs, GetOutpostInstanceTypeResult, GetOutpostInstanceTypeOutputArgs } from "./getOutpostInstanceType";
export declare const getOutpostInstanceType: typeof import("./getOutpostInstanceType").getOutpostInstanceType;
export declare const getOutpostInstanceTypeOutput: typeof import("./getOutpostInstanceType").getOutpostInstanceTypeOutput;
export { GetOutpostInstanceTypesArgs, GetOutpostInstanceTypesResult, GetOutpostInstanceTypesOutputArgs } from "./getOutpostInstanceTypes";
export declare const getOutpostInstanceTypes: typeof import("./getOutpostInstanceTypes").getOutpostInstanceTypes;
export declare const getOutpostInstanceTypesOutput: typeof import("./getOutpostInstanceTypes").getOutpostInstanceTypesOutput;
export { GetOutpostsArgs, GetOutpostsResult, GetOutpostsOutputArgs } from "./getOutposts";
export declare const getOutposts: typeof import("./getOutposts").getOutposts;
export declare const getOutpostsOutput: typeof import("./getOutposts").getOutpostsOutput;
export { GetSiteArgs, GetSiteResult, GetSiteOutputArgs } from "./getSite";
export declare const getSite: typeof import("./getSite").getSite;
export declare const getSiteOutput: typeof import("./getSite").getSiteOutput;
export { GetSitesResult } from "./getSites";
export declare const getSites: typeof import("./getSites").getSites;
export declare const getSitesOutput: typeof import("./getSites").getSitesOutput;
